  ...
  <div class="sticky top-0 flex z-50 bg-white h-16 border-b-2 border-gray-100">
    <div class="grid grid-cols-3 gap-4 items-center p-2 md-5 w-full">
      ...
      <div class="flex gap-x-2 md:gap-x-5 items-center justify-end mr-2 md:mr-4">
        <button v-if="!user" @click.stop="toggleLoginPopup" class="text-gray-400 md:text-base text-sm hover:text-gray-700">Log in</button>
        <router-link class="text-gray-400 md:text-base text-sm hover:text-gray-700" to="/about">About</router-link>
        <div v-if="user" @click.stop="toggleAccountPopup" class="flex text-gray-400 md:text-base text-sm hover:text-gray-700 items-center">
          <user-circle-icon class="h-6 w-6 mt-0.5 transform transition-transform"/>
        </div>
      </div>
    </div>
  </div>
  <div v-show="isDropdownActive" v-click-outside="disableDropdown" class="absolute z-50 top-16 w-screen md:ml-4 px-6 py-4 mt-1 md:max-w-sm shadow-lg bg-white border-t-2 border-green-100">
