<template>
  <div id="sidebar" class="fixed z-40 inset-0 flex-none h-full bg-opacity-25 w-full lg:static lg:h-auto lg:overflow-y-visible lg:pt-0 lg:w-60 xl:w-72 lg:block hidden">
        ...
        <ul>
          <li v-for="(section, i) in sections" :key="i" :class="{ activeClass: sectionId == i + 1 }" class="hover:bg-gray-300 transition-colors duration-200">
            <div class="flex items-center">
              <div class="w-4 h-4 rounded-full border-2 shadow-sm border-gray-300 bg-white flex justify-center items-center">
                <div class="w-2 h-2 rounded-full bg-green-300"></div>
              </div>
              <router-link class="px-3 py-2 relative block text-gray-900" :to="{ name: 'Section', params: { sectionId: section.index } }">{{ section.title }}</router-link>
            </div>
          </li>
        </ul>
      </nav>
